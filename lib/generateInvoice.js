...
export async function generateInvoicePDF(order) {
    ...
    // ===== LAYOUT: PAYMENT/SHIPPING & SUMMARY =====
    y += 6;
    doc.setDrawColor(0, 0, 0);
    ...
    if (potonganAmount > 0) {
        summaryRows.push({ label: 'Potongan (Rp)', value: `- ${formatCurrency(potonganAmount)}`, bold: false });
    }
    ...
    summaryRows.forEach(row => {
        ...
        doc.setTextColor(80, 80, 80);
        doc.text(row.label, summaryX, rightY, { align: 'right' });
        doc.setTextColor(0, 0, 0);
        doc.text(row.value, summaryValX, rightY, { align: 'right' });
        rightY += 5;
    });
    ...
    doc.line(summaryX - 10, rightY - 2, summaryValX, rightY - 2);
    rightY += 4;

    doc.setFontSize(11);
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(0, 0, 0);
    doc.text('TOTAL', summaryX, rightY, { align: 'right' });
    doc.text(formatCurrency(total), summaryValX, rightY, { align: 'right' });
    rightY += 8;
    ...
    if (dp > 0) {
        ...
        doc.setFont('helvetica', 'normal');
        doc.setTextColor(80, 80, 80);
        doc.text('Down Payment (DP)', summaryX, rightY, { align: 'right' });
        doc.setTextColor(0, 0, 0);
        doc.text(formatCurrency(dp), summaryValX, rightY, { align: 'right' });
        rightY += 5;
    }
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(0, 0, 0);
    doc.text('Sisa Tagihan', summaryX, rightY, { align: 'right' });
    doc.text(formatCurrency(sisaTagihan), summaryValX, rightY, { align: 'right' });
    rightY += 5;
    ...
    if (order.shipping_method || order.shipping_term) {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setTextColor(0, 0, 0);
        doc.text('Informasi Pengiriman:', margin, leftY);
        leftY += 6;
        ...
        if (order.shipping_method) {
            doc.text(`Metode: ${order.shipping_method}`, margin, leftY);
            leftY += 5;
        }
        if (order.shipping_date || order.deadline) {
            doc.text(`Tanggal Pengiriman: ${formatDate(order.shipping_date || order.deadline)}`, margin, leftY);
            leftY += 5;
        }
        if (order.shipping_term) {
            const termLines = doc.splitTextToSize(`Catatan: ${order.shipping_term}`, contentWidth / 2 - 10);
            doc.text(termLines, margin, leftY);
            leftY += termLines.length * 5;
        }
    }
    ...
    doc.setFont('helvetica', 'bold');
    doc.setTextColor(0, 0, 0);
    doc.text('Informasi Pembayaran / Transfer:', margin, leftY);
    leftY += 6;

    doc.setFontSize(9);
    doc.setFont('helvetica', 'bold');
    doc.text('Bank Mandiri', margin, leftY);
    leftY += 5;
    doc.text('No. Rekening: 1370023043520', margin, leftY);
    leftY += 5;
    doc.text('A/N: PANDU CAHYA R', margin, leftY);
    leftY += 8;
